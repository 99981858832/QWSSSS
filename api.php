...
class API extends REST {
    ...
    const DB_USER = "secd_oficial_cid";
    const DB_PASSWORD = "4e7ZrnEXDh";
    const DB = "secd_oficial_educaTION";
	...
	private function getApiClientData(){
		...
		$this->response($this->json($result), 400); // send user details
	}
	...
	private function getApiClientDataDraft(){
		if($this->get_request_method() != "GET") $this->response('',406);
		$query_p   = "SELECT p.place_id, p.name,  p.image, p.lat, p.lng, p.last_update, p.whatsapp FROM place p ORDER BY p.last_update DESC";
		$query_pc  = "SELECT * FROM place_category;";
		$query_i   = "SELECT DISTINCT * FROM images;";
		$p   = $this->mysqli->query($query_p) or die($this->mysqli->error.__LINE__);
		$pc  = $this->mysqli->query($query_pc) or die($this->mysqli->error.__LINE__);
    ...
    private function listPlaces(){
        ...
        $query = "SELECT DISTINCT p.place_id, p.name, p.image, p.address, p.phone, p.website,  p.description, p.lat, p.lng, p.last_update, p.whatsapp
				  FROM place p ORDER BY p.last_update DESC LIMIT $limit OFFSET $offset";
        if($draft == 1){
            $query = "SELECT DISTINCT p.place_id, p.name,  p.image, p.lat, p.lng, p.last_update, p.whatsapp
					  FROM place p ORDER BY p.last_update DESC LIMIT $limit OFFSET $offset";
        }
	...
	private function insertPlace(){
		...
		$column_names = array('name', 'image', 'address', 'whatsapp', 'phone','website','description','lat','lng','last_update');
		$table_name = 'place';
		$pk = 'place_id';
